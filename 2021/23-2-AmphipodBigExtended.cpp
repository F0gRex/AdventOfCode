...
static vector<vector<vector<int>>> bestPath;
static vector<vector<vector<int>>> curPath;
...
void printState(vector<vector<int>> const &burrow);
...
// prints the provided state with the same format as the input
void printState(vector<vector<int>> const &burrow) {
    cout << "#############" << endl
         << "#";
    for (int j = 0; j < 11; j++) {
        if (burrow[0][j] == 0)
            ...
            cout << char(burrow[0][j] + 64);
    }
    cout << "#" << endl;

    for (int i = 1; i < burrow.size(); i++) {
        cout << ((i == 1) ? "###" : "  #");
        for (int j = 2; j < 9; j++) {
            if (burrow[i][j] == 0)
                cout << ".";
            else if (burrow[i][j] == -1)
                cout << "#";
            else
                cout << char(burrow[i][j] + 64);
        }
        if (i == 1)
            cout << "##";
        cout << "#" << endl;
