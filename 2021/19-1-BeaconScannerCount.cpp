...
int main(int argc, char const *argv[]) {
    ...
    unordered_set<string> beacons;
    for (int i = 0; i < n; i++) {
        int r = scPosition[i][3];
        for (int j = 0; j < rotScanners[r][i].size(); j++) {
            string id = to_string(rotScanners[r][i][j][0] + scPosition[i][0]) + "#" 
                      + to_string(rotScanners[r][i][j][1] + scPosition[i][1]) + "#" 
                      + to_string(rotScanners[r][i][j][2] + scPosition[i][2]);
            beacons.insert(id);
        }
    }
